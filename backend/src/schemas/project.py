from uuid import UUID
from datetime import datetime
from typing import List, Optional
from pydantic import BaseModel, ConfigDict, Field
from typing import Literal

from .column import ColumnRead
from .task import TaskRead
from .user import UserRead


class ProjectBase(BaseModel):
    name: str = Field(..., max_length=100)
    description: str
    status: Literal['активный', 'архивный'] = 'активный'


class ProjectCreate(ProjectBase):
    pass


class ProjectUpdate(BaseModel):
    name: str | None = Field(None, min_length=1, max_length=100)
    description: Optional[str] = None
    status: str | None = Field(None, min_length=1, max_length=20)


class ProjectRead(ProjectBase):
    """Ответ API"""
    id: UUID
    created_at: datetime
    updated_at: datetime
    
    # Вложенные связи (с пустыми списками по умолчанию)
    # Важно: эти схемы должны быть определены ДО ProjectRead
    columns: List["ColumnRead"] = []
    tasks: List["TaskRead"] = []
    members: List["UserRead"] = [] # Зависит от того, как названа схема участника

    model_config = ConfigDict(from_attributes=True)


class ProjectInDB(ProjectRead):
    """Соответствует полям UsersOrm (в т.ч. для чтения из сессии)."""

    pass

