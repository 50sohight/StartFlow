<script>
  ...
  function openProject(name) {
    // Если name — это project.id (UUID), лучше использовать его
    // но раз у вас name, то ок
    goto(`/projects/${encodeURIComponent(name)}/board`);
  }
</script>
...
<div class="min-h-[calc(100vh-64px)] flex flex-col lg:flex-row">
        ...
        {:else if projects.length === 0}
          <div class="text-center py-12">
            ...
            <a
              ...
              class="inline-block py-3 px-6 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition shadow-md"
            >
              Создать проект
            </a>
          </div>
        {:else}
          <div class="grid grid-cols-1 gap-4">
            {#each projects as project}
              <button
                on:click={() => openProject(project.name)}
  ...
  <!-- Декоративная правая часть (как в референсе) -->
